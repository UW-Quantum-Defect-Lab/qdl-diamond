...
from qdlutils.experiments.laser_pulse_sequencing.repump_pump_probe_sequence_base import (
    RepumpPumpProbeSequenceBase
)
...
class PumpProbeT1Experiment(RepumpPumpProbeSequenceBase):

    '''
    This class implements a two-laser T1 experimental sequence. The experimental protocol is
    comprised of three lasers "repump", "pump", and "probe". The repump is periodically pulsed to 
    initialize the system state (typically recharge + scramble the state). The pump signal 
    coherently drives population and the probe reads out the state. Graphically:

    ```
        repump    __|=======|____________________________________________

        pump      ______________|===========|____________________________

        probe     __________________________________|===========|________
    ...
    def get_sequence_output_data(
        ...
        # Get the times associated to the relevant points in the sequence in terms of clock cycles
        idx1 = int(repump_time * self.clock_rate)
        idx2 = idx1 + int(pump_delay * self.clock_rate)
        idx3 = idx2 + int(pump_time * self.clock_rate)
        idx4 = idx3 + int(probe_delay * self.clock_rate)
        idx5 = idx4 + int(probe_time * self.clock_rate)
        self.single_sequence_n_samples = idx5 + int(end_delay * self.clock_rate)
        ...
        self.single_sequence_repump_data = np.zeros(self.single_sequence_n_samples, dtype=np.int32)
        self.single_sequence_repump_data[0:idx1] = 1
        self.single_sequence_pump_data = np.zeros(self.single_sequence_n_samples, dtype=np.int32)
        self.single_sequence_pump_data[idx2:idx3] = 1
        self.single_sequence_probe_data = np.zeros(self.single_sequence_n_samples, dtype=np.int32)
        self.single_sequence_probe_data[idx4:idx5] = 1
