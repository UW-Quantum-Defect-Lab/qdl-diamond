...
from qdlutils.experiments.laser_pulse_sequencing.repump_probe_sequence_base import RepumpProbeSequenceBase
...
class RepumpPumpProbeSequenceBase(RepumpProbeSequenceBase):

    '''
    This class implements the basic features for pulse sequence experiments involving a repump 
    laser, pump laser, and probe laser.

    The repump laser does not have any frequency control but is turned on/off via a TTL digital
    output signal.

    The pump laser (in this iteration) does not have any direct frequency control via the code but
    can be scanned via other software. It is also turned on/off via a TTL digital output signal. In
    future interations where direct control is available, users must implement similar methods to
    the probe laser methods.
    ...
    We are inheriting all of the features from the `RepumpProbeSequenceBase` class so we don't have
    to track changes to the more core functionalities. In the long term it may be better to rewrite
    the code such that there is an intermediate class from which both of these sequences inherit
    from.
    '''
    ...
    def __init__(
        ...
        self.single_sequence_pump_data = None
        ...
        # Kind of hack-y but want to init from the SequenceControllerBase rather than the repump
        # probe sequence base class. Here just directly call the constructor.
        SequenceControllerBase.__init__(
            self,   # Needs to reference this instance to init
    ...
    def set_pump_switch(
        ...
        '''
        Set the pump laser switch on or off.
        '''
        self.set_output(output_id=self.pump_id, setpoint=setpoint)
    ...
    def run(
        ...
        '''
        This method is rewritten here to manage the data for the pump. Note that changes between
        this and the `RepumpProbeSequenceBase` class will not be carried over.
        ...
        **kwargs
        ...
        self.output_data = {
            ...
            self.pump_id          : np.tile(self.single_sequence_pump_data, self.n_repetitions),
    ...
    def single_probe_scan(
        ...
        pump_data = np.zeros(n_samples)     # Always off
